"""
Database access for the Provisioning Service — Azure SQL (sqldb-provisioning),
Entra-only auth (no SQL logins). Added in Phase 3.5: the task-state store
behind the admin console's provisioning queue (list/retry/cancel). The
database was provisioned in data.bicep's serviceDatabases list from day one —
this pass finally uses it.
...
SQL_DATABASE = os.environ.get("SQL_DATABASE", "sqldb-provisioning")
