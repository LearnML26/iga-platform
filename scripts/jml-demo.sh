...
#     no delete endpoint, so this script aborts if the fixture's first
#     established key already exists — bump FIRST_ID/KEY_PREFIX in
#     generate_fixtures.py to rerun. Every key this script checks is
#     sourced from fixture_keys.env (regenerated by generate_fixtures.py
#     alongside the CSVs, step 0 below) rather than hardcoded — a prior
#     revision hardcoded J1xxx literals, which silently broke every
#     assertion the moment KEY_PREFIX/FIRST_ID were bumped for a rerun.
# ============================================================================
set -uo pipefail
FAIL=0
ok()   { echo "  ✔ $1"; }
...
source "$FIXDIR/fixture_keys.env" || { bad "fixture_keys.env missing/unreadable — regenerate failed silently?"; exit 1; }
ok "fixtures regenerated (scheduled terminations: $SCHED_TERM_KEY_1 -> $SCHED_TERM_DATE_1, $SCHED_TERM_KEY_2 -> $SCHED_TERM_DATE_2)"
...
OUT=$(get_ident guard "$FIRST_ESTABLISHED_KEY")
if echo "$OUT" | grep -q 'HTTP_STATUS:200'; then
  bad "$FIRST_ESTABLISHED_KEY already exists — this demo was run before. correlationKey is global with no"
  echo "    delete endpoint, so rerunning would report updates instead of adds. Bump FIRST_ID or"
  echo "    KEY_PREFIX in fixtures/jml-demo/generate_fixtures.py and rerun."
  exit 1
fi
ok "key space clear ($FIRST_ESTABLISHED_KEY not present)"
...
OUT=$(get_ident "$(echo "$FIRST_ESTABLISHED_KEY" | tr '[:upper:]' '[:lower:]')" "$FIRST_ESTABLISHED_KEY")
echo "$OUT" | grep -q '"status":"active"' && ok "$FIRST_ESTABLISHED_KEY (established) is active" \
  || bad "$FIRST_ESTABLISHED_KEY not active: $OUT"
for K in $JOINER_KEYS; do
...
for K in $JOINER_INSIDE_WINDOW; do
...
for K in $JOINER_OUTSIDE_WINDOW; do
  OUT=$(get_ident "$(echo "$K" | tr '[:upper:]' '[:lower:]')-b" "$K")
...
for K in "$SCHED_TERM_KEY_1" "$SCHED_TERM_KEY_2"; do
  OUT=$(get_ident "$(echo "$K" | tr '[:upper:]' '[:lower:]')" "$K")
...
for K in $TRANSFER_KEYS; do
  OUT=$(get_ident "$(echo "$K" | tr '[:upper:]' '[:lower:]')" "$K")
  echo "$OUT" | grep -q '"status":"active"' && ok "$K (transfer) still active with updated attributes" \
    || bad "$K wrong state: $OUT"
done

echo ""
echo "== Day-0 demo complete — what happens next, on its own =="
echo "  - The lifecycle-sweep CronJob (daily 06:00 UTC) will terminate:"
echo "      $SCHED_TERM_KEY_1 on $SCHED_TERM_DATE_1, $SCHED_TERM_KEY_2 on $SCHED_TERM_DATE_2"
...
echo "        curl -sS -H \"Authorization: Bearer \$TOKEN\" http://identity-service/identities/by-correlation-key/$SCHED_TERM_KEY_1"
echo "    (or re-run the sweep manually on/after those dates: POST /lifecycle/sweep)"
echo "  - The 2 disable-account tasks just dispatched for $DROPPED_KEYS will retry ~2.5h"
